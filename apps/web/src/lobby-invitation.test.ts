...
describe("browser lobby invitation profile", () => {
  it("round trips exact fragment fields while preserving the deployment path", () => {
    ...
    expect(url).toBe(`https://example.test/cards/#g=${"11".repeat(16)}&h=${Array.from(HOST, (v) => v.toString(16).padStart(2, "0")).join("")}&r=sasku-match-candidate%403&s=trystero-nostr`);
  ...
  it("rejects unsupported rules, strategies, protocols and weak identities", () => {
    ...
    for (const bad of [valid.replace("sasku-match-candidate%403", "sasku%401"), valid.replace("trystero-nostr", "manual"), valid.replace("https:", "javascript:"), valid.replace(/h=[0-9a-f]+/, `h=${"00".repeat(32)}`), "x".repeat(4097)]) {
