...
use ark_ff::{Field};
use ark_std::{borrow::BorrowMut, vec::Vec, rand::Rng};
    ...
    Comm: HomomorphicCommitmentScheme<Scalar>,
{
    ...
    pub fn prove<R: Rng>(
        ...
        let vector_of_committed_diagonals = diagonals
            .iter()
            .zip(t.iter())
            .map(|(&diagonal, &random)| -> CryptoResult<_> {
                Comm::commit(self.parameters.commit_key, &vec![diagonal], random)
            })
            .collect::<CryptoResult<Vec<_>>>()?;

        // Public parameters
        self.parameters.transcript_append(tt);

        // Random values
        tt.append(&a_0_commit);
        tt.append(&b_m_commit);

        // Commitments
        tt.append(self.statement.commitment_to_a);
        tt.append(self.statement.commitment_to_b);
        tt.append(&vector_of_committed_diagonals);

        let x: Scalar = tt.challenge(b"x").read_uniform();
