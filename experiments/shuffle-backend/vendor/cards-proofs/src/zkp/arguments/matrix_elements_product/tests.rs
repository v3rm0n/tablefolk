...
mod test {
    use crate::{
        ...
        zkp::{arguments::matrix_elements_product, ArgumentOfKnowledge},
    };
    ...
    fn test_complete_product_argument() {
        ...
        assert_eq!(
            Ok(()),
            valid_proof.verify(&proof_parameters, &statement, seed)
        );
        ...
        assert_ne!(
            Ok(()),
            invalid_proof.verify(&proof_parameters, &statement, seed)
        );
        ...
        assert_ne!(
            Ok(()),
            invalid_proof.verify(&proof_parameters, &wrong_statement, seed)
        );
    }
}
