...
use crate::{
    ...
    utils::{
        rand::sample_vector,
        vector_arithmetic::{dot_product, hadamard_product},
    },
    vector_commitment::HomomorphicCommitmentScheme,
    zkp::{
        arguments::{zero_value_bilinear_map, zero_value_bilinear_map::YMapping},
        ArgumentOfKnowledge,
    },
};
    ...
    Comm: HomomorphicCommitmentScheme<Scalar>,
{
    ...
    pub fn prove<R: Rng+CryptoRng>(
        ...
        let b_commit_middle = b
            ...
            .skip(1)
            .zip(s.iter())
            .map(|(b_i, &s_i)| -> CryptoResult<Comm::Commitment> {
        ...
        // Public parameters
        self.parameters.transcript_append(t);
        ...
        let temp_x_b = b[1..=self.parameters.m - 1]
            ...
            .collect::<CryptoResult<Vec<Vec<Scalar>>>>()?;
