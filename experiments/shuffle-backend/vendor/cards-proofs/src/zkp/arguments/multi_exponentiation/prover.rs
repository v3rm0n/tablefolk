...
use crate::{
    ...
    utils::{rand::sample_vector, vector_arithmetic::dot_product},
    vector_commitment::HomomorphicCommitmentScheme,
    zkp::arguments::scalar_powers,
};

use ark_ff::{Field, Zero};
use ark_std::{borrow::BorrowMut, marker::PhantomData, vec::Vec, rand::{Rng, CryptoRng}};
    ...
    Comm: HomomorphicCommitmentScheme<Scalar>,
{
    ...
    pub fn prove<R: Rng+CryptoRng>(
        ...
        t.append(self.statement.commitments_to_exponents);
        t.append(&self.statement.product);
        t.append(self.statement.shuffled_ciphers);
        ...
        let commit_b_k = b
            ...
            .collect::<CryptoResult<Vec<Comm::Commitment>>>()?;
    ...
    fn diagonals_from_chunks(
        cipher_chunks: &[Vec<Enc::Ciphertext>],
        scalar_chunks: &[Vec<Scalar>],
        a_0_randomness: &[Scalar],
    ) -> CryptoResult<Vec<Enc::Ciphertext>> {
        ...
        let product: CryptoResult<Vec<Enc::Ciphertext>> = cipher_chunks
            .iter()
