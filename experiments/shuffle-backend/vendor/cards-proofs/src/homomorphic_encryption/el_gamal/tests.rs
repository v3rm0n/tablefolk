...
mod test {
    ...
    use ark_std::{vec::Vec, rand::thread_rng};
    use std::ops::Mul;
    ...
    fn valid_encrypt_decrypt() {
        ...
        let parameters = ElGamal::setup(rng);
        let (pk, sk) = ElGamal::keygen(&parameters, rng);
        ...
        let encrypted = messages
            .iter()
            .zip(randoms.iter())
            .map(|(m, r)| ElGamal::encrypt(&parameters, &pk, m, r))
            .collect::<Vec<_>>();

        let decrypted = encrypted
            .iter()
            .map(|c| ElGamal::decrypt(&parameters, &sk, c))
            .collect::<Vec<_>>();
    ...
    fn failed_encrypt_decrypt() {
        ...
        let parameters = ElGamal::setup(rng);
        let (pk, _) = ElGamal::keygen(&parameters, rng);
        ...
        let decrypted = encrypted
            .iter()
            .map(|c| ElGamal::decrypt(&parameters, &wrong_sk, c))
            .collect::<Vec<_>>();
    ...
    fn homomorphic_property() {
        ...
        let c1 = ElGamal::encrypt(&parameters, &pk, &m1, &r1);

        let m2 = Plaintext::rand(rng);
        let r2 = Scalar::rand(rng);
        let c2 = ElGamal::encrypt(&parameters, &pk, &m2, &r2);
        ...
        let decrypted = ElGamal::decrypt(&parameters, &sk, &c3);
